...
- **FUNCIONANDO (2026-07-14)** ✅ — verificado end-to-end: recovery a `enrique@dreamtec.cl` → Resend `last_event: delivered`, remitente **Dreamtec Experiencias**, **logo Dreamtec**, y enlace a la **app** `https://expdt.verticecorp.cl/?th=…&vt=recovery` (no el portal). Se resolvieron dos secrets del proyecto pqhc vía Management API: `RESEND_API_KEY` (estaba inválido) y `MAIL_FROM` (estaba con formato inválido → se dejó `Dreamtec Experiencias <no-reply@verticecorp.cl>`; esto también destraba los correos de `notify-task`). El remitente del auth-email va **hardcodeado** en la función (no depende de `MAIL_FROM`).
