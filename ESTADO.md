...
- **FALTA (deploy)**: desplegar la Edge Function `auth-email` en el proyecto Supabase (Dashboard → Edge Functions → Deploy new function → pegar `index.ts`, o `supabase functions deploy auth-email`). Asegurar el secret `RESEND_API_KEY` (si notify-task ya está, ya existe). No requiere Verify-JWT off (se llama con la anon key).
